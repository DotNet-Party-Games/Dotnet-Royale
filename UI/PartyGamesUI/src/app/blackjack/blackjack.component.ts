...
export class BlackjackComponent implements OnInit, AfterViewInit {
  ...
  readyPlayers() {
    for(let i = 0; i < this.players.length; i++) {
      ...
      this.gameState.players.push(bj);
    }
  }
  ...
  draw(i : number) {
    ...
    if(i == 42) {
      ...
      this.gameState.dealer.push(card);
    }
    ...
    // (D3) PLAYER'S CARD
      ...
      this.gameState.players[i].player.push(card);
    }
  }
  ...
  hit(i : number) {
    ...
    if(i == 42) {
      ...
      this.draw(i); 
      this.points(i);
      this.socketService.sendBlackJackData(this.gameState);
