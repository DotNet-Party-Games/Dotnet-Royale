...
import { SnakeData } from './snakedata.service';
...
export class SnakeComponent implements OnInit {
  ...
  constructor(private snakeData : SnakeData) { }
  ...
  private setupGame(){
    const direction = fromEvent(document,'keydown').pipe(
      ...
      tap((event:KeyboardEvent)=> this.snakeData.reduce(state=> directionReducer(state,event))),
    );
    const tick = interval(TICK_INTERVAL).pipe(
      tap(() => this.snakeData.reduce(tickReducer)),
      );
    const game = merge(direction,tick);

    const gameOver = this.snakeData.select(state => state.game.gameOver).pipe(
